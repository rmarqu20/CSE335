...
class cities
{
    ...
    //create a new city
    func editCity(cName: String?, cDescription: String?, cImage: UIImage)
    {
        fetch()
        for city in fetchResults
        {
            ...
            {
                city.cityImage = cImage.jpegData(compressionQuality: 0.8) as NSData?
                city.cityDescription = cDescription
            }
        }
        do
        {
            ...
            {
                print("TEST4")
                self.fetchResults = try self.managedObjectContext.fetch(request)
            }
            catch
            {
